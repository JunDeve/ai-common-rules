...
- **High-Density Tone**:
  ...
  - **No Redundant Paraphrasing**:
    - 출력된 코드 블록, 로그, 도구 실행 결과 등 사용자가 직접 확인할 수 있는 원본 데이터(Raw Data)를 다시 서술하거나 요약하지 않습니다.
    - 이미 제시한 결론, 계획, 상태 보고를 다른 표현으로 반복하지 않습니다.
  - **Delta-Focused**:
    - 답변은 수행 과정 전체의 나열이 아니라, 변경점(Delta)과 다음 단계에만 집중합니다.
    - 진행 중 업데이트는 새롭게 확인된 Delta만 전달합니다.
...
- **Post-Action Reporting Discipline**:
  - 작업 완료 후 보고에서는 "무엇이 실제로 변경되었는가"만 기술합니다.
  - 작업 전 계획 문장을 동일 표현으로 반복하지 않고, 계획 대비 실제 차이점만 보고합니다.
...
- **High-Density Tone**:
  ...
  - **No Redundant Paraphrasing**:
    - Do not restate or summarize raw data that is already visible to the user, including code blocks, logs, and tool outputs.
    - Do not repeat an already stated conclusion, plan, or status update in different wording.
  - **Delta-Focused**:
    - Focus on delta and next steps instead of listing the full process.
    - Interim updates must contain only newly discovered delta.
...
- **After Action Reporting**:
  - **Actual Changes**: Summary of the applied code and modifications
- **Approval Response Discipline**:
...
- **Post-Action Reporting Discipline**:
  - After execution, report only what actually changed.
  - Do not repeat the pre-action plan using the same wording. Report only the delta between plan and actual result.